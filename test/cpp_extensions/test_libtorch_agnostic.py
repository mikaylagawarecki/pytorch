...
if not IS_WINDOWS:
    ...
    class TestLibtorchAgnostic(TestCase):
        ...
        @skipIfTorchVersionLessThan(2, 10)
        @parametrize("layout", [None, torch.strided])
        @parametrize(
        ...
        def test_my_empty(self, device, layout, memory_format):
            ...
            try:
                ...
                # Test that unsupported layouts raise an error (only once)
                if layout is None and memory_format is None:
                    with self.assertRaises(RuntimeError):
                        libtorch_agnostic.ops.my_empty(
                            [2, 3], torch.float32, torch.sparse_coo, device, None, None
                        )
            finally:
                torch.use_deterministic_algorithms(deterministic)
