...
int64_t test_get_current_device() {
  return torch::stable::accelerator::getCurrentDevice();
}

void boxed_test_get_current_device(
    StableIValue* stack,
    uint64_t num_args,
    uint64_t num_outputs) {
  int64_t res = test_get_current_device();
  stack[0] = from(res);
}
...
STABLE_TORCH_LIBRARY_FRAGMENT(libtorch_agnostic, m) {
  ...
  m.def("test_get_current_device() -> int");
}
...
STABLE_TORCH_LIBRARY_IMPL(libtorch_agnostic, CompositeExplicitAutograd, m) {
  ...
  m.impl("test_get_current_device", &boxed_test_get_current_device);
}
#endif // LAE_USE_CUDA
